...
- ```ServletContextListener```: 웹 애플리케이션 생성에서 소멸까지의 주요사항 모니터링
	- 개별 서블릿 동작 전 application 시작 시 초기화하는데 비용이 많이 드는 공유자원의 초기화에 주로 사용

## 5-1. 등록
- ```@WebListener()`` 을 이용한 등록
- ```web.xml``` 을 이용한 등록
...
### 뮤텍스
- 임계 구역에 들어갈 수 있는 통로가 단 하나
- ```synchronized```
### 세마포어
- 임계 구역에 들어갈 수 있는 통로가 여러 개
- ```connection pool```

- Servlet 은 multi thread 에 의해 공유됨
	- Servlet 은 공유자원으로 thread safe 하지 않음
- 데이터 보호를 위해 synchronized 키워드 사용: 한 공유자원에 대해 한 번에 한 공유자원만 접근 가능
